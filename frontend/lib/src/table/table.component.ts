...
export class TableComponent implements OnInit, OnChanges {
  ...
  tableToExcel(id) {
    ...
    setTimeout(() => {
      ...
      save(new Blob([tab_text], { type: "application/vnd.ms-excel" }),  "CAP_" + new Date().getTime().toString() + ".xls" );
      setTimeout(() => {
        this.itemsPerPage = itemsPerPage;
      }, 500)
    }, 200)
  ...
  indicatorValue(inputObj, key, columnData, e){
      if(inputObj[key] > 100 ){
        
        inputObj[key] = inputObj[key].toString().substring(0, 1);
          $(this).val(inputObj[key]);
      
      } else if(inputObj[key]==0){
        inputObj[key] = "";
      }
      // if (!inputObj[key].Text.RegularExpressions.Regex.IsMatch(this.textbox.Text, "^[0-9]*$")) {
      //   return true;
      // }  
   
  }
  emit(rowDetails,col){
    if(rowDetails[col] > 100){
      rowDetails[col]= rowDetails[col].toString().substring(0, 1);
      $(this).val(rowDetails[col]);
    }
    else if(rowDetails[col] == 0){
      rowDetails[col] = "";
    }
    let letters = /^\d{1,6}(\.\d{1,1})?$/;
    if(!rowDetails[col].match(letters)){    
      rowDetails[col] = "";
    }
  }   
}