...
/**
  ...
  selectIndicators()
  {
    ...
    this.selectedIndicators.forEach(element =>{
      ...
      {
        this.selectedThemeIndicator.get(element.sc[0].sectorName).add(element.name);
      }
      else
      {
        let indicators :Set<String> =new Set();
        indicators.add(element.name);
  ...
  next()
  {
    ...
    this.selectedArea= <IArea>(this.areaLevel?(this.areaLevel.stateAvailable?
      (this.areaLevel.districtAvailable?this.district:
     this.state):ComparisonViewAreaFilterPipe.prototype.transform(this.area,this.areaLevel,"GLO")[0]):null);

     if(this.areaLevel.stateAvailable&&!this.state){
      alert.setSubTitle("Select State");
      alert.present();
    }
    else if(this.areaLevel.districtAvailable&&!this.district)
    {
