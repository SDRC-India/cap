...
export class CustomViewVisualizationViewPage {
  ...
  visualizations: String[] = ["Line Chart"]
  ...
  async getCustomViewVisualizationData() {
    ...
    if (!this.customViewTypeSelection.sourceWise) {
      // switch(this.selectedArea.areaLevel[0].level)
      // {
      ...
      //     break;
      //   case AreaLevel.BLOCK:
      //   this.selectedSource=this.customViewVisualizationData.has(this.indicator.block.sourceName)
      //   ?this.indicator.block.sourceName:this.getKeys(this.customViewVisualizationData)[0]

      //     break;
      // }
      this.selectedSource = this.getKeys(this.customViewVisualizationData)[0];
    }
    else {
  ...
  selectSource() {
    ...
    if (this.selectedView != 3) {
      ...
      visualizationLine.baseLine = this.dbData.filter(obj => obj.baseline == true)[0].value;
      visualizationLine.target = this.dbData.filter(obj => obj.baseline == true)[0].target;
      visualizationLine.tp = this.dbData.filter(obj => obj.baseline == true)[0].tp;
      visualizationLine.src = this.dbData.filter(obj => obj.baseline == true)[0].src.sourceName;
