...
export class CustomViewDataTablePage {
  ...
  indicatorList: IIndicator[];
   ...
   selectedSourceObject:IDepartment
  ...
  ngOnInit() {
    ...
    let maternal: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().themeName_maternal).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
    ...
    let reproductive: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().themeName_reproductive).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
      } else if (a.name < b.name) {
        return -1
    ...
    let childcare: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().themeName_childcare).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
      } else if (a.name < b.name) {
        return -1
    ...
    let neonatal: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().themeName_neonatal).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
      } else if (a.name < b.name) {
        return -1
    ...
    let infra: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().health_infrastructure).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
      } else if (a.name < b.name) {
        return -1
    ...
    let hr: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().human_resource).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
      } else if (a.name < b.name) {
        return -1
    ...
    let adlosecent: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().adolescent).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
      } else if (a.name < b.name) {
        return -1
    ...
    let demography: IIndicator[] = this.indicatorList.filter(d => d.recSector.name === this.constantService.getConstantObject().demography).sort((a: IIndicator, b: IIndicator) => {
      if (a.name > b.name) {
        return 1
      } else if (a.name < b.name) {
        return -1
  ...
  kPIIndicator.forEach(element=>{
      indicators.push(element.id);
        this.sectorsMap.set(element.recSector.id,element.recSector);
    });
    ...
    nonKPIIndicator.forEach(element=>{
      nonKPIIndicators.push(element.id);
      this.sectorsMap.set(element.recSector.id,element.recSector);
    });
    ...
    this.getAreaWiseComparisionData(this.selectedArea.code,indicators,nonKPIIndicators)
      ...
      let sector: ISector = this.navParams.get("sector");
      let areas=[];
      ...
      this.getIndicatorWiseComparision(areas,this.selectedIndicator.id,isOffline, sector);
    }
    ...
    else
    {
      ...
      let isOffline:boolean=this.selectedIndicator.kpi||this.selectedIndicator.nitiaayog||this.selectedIndicator.thematicKpi||this.selectedIndicator.hmis||this.selectedIndicator.ssv;;
      this.getSourceWiseComparision(areas,this.selectedIndicator.id,isOffline,this.selectedSourceObject.id);
    }
  ...
  async getIndicatorWiseComparision(areas:string[],indcatorId:Number,isOffline:boolean, sector: ISector)
  {
    this.geographyWiseComparision=await this.customViewServiceProvider.getIndicatorWiseComparision(areas,indcatorId,sector.id, true);
  }

  async getSourceWiseComparision(areas:string[],indcatorId:Number,isOffline:boolean,source :number)
  {
    this.geographyWiseComparision=await this.customViewServiceProvider.getSourceWiseComparision(areas,indcatorId,true,source);
   // this.selectedSource=this.selectedSourceObject.name;
  }
      /**
  ...
  {
    if(this.geographyWiseComparision.values.has(indicator.name))
    this.navCtrl.push("CustomViewVisualizationViewPage",{
      "area":this.selectedArea,"customViewTypeSelection":
      this.customViewTypeSelection,"indicator":indicator,"data":this.geographyWiseComparision.indicatorData.get(indicator.name),"source":this.selectedSource})
    else
    {
  ...
  {
    if(this.geographyWiseComparision.values.has(area.areaname)&&(this.customViewTypeSelection.indicatorWise))
