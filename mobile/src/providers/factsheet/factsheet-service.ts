...
export class FactsheetServiceProvider {
  ...
  async intializeFactsheetDatabase() {
    ...
    let ahsData: FactsheetData = {
      source: "AHS",
      round: [
        {
          sourceName: "AHS 12-13",
          url: "doc/AHS_report_part1.pdf"
        }
      ]
    };
    factsheetDatas.push(ahsData);
  ...
  async getFactSheetData() {
    ...
    // );
    let doc = await this.http.get("assets/data/factsheet.json").toPromise();
  ...
  async updateSource() {
    ...
    // );
    let doc = await this.http.get("assets/data/sourceIndicator.json").toPromise();
