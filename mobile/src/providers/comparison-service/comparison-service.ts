...
export class ComparisonServiceProvider {
   ...
   */
  async getArea() {
    let doc: Area[] = await this.http.get<Area[]>("assets/data/area.json").toPromise();
    let areas: Area[] = [];
    for (let i = 0; i < doc.length; i++) {
      let level = [];
        level.push(doc[i].areaLevel[0].level);
      let area: Area = {
        areaname: doc[i].areaname,
        parentAreaCode: doc[i].parentAreaCode,
        code: doc[i].code,
        levels: level,
        id: doc[i].id,
        areaLevel: doc[i].areaLevel,
        slugidarea: doc[i].slugidarea
      };
      areas.push(area);
    }
   ...
   */
  async getAreaLevel() {
    let areaLevels: IAreaLevel[] = await this.http.get<IAreaLevel[]>("assets/data/arealevel.json").toPromise();
  ...
  async getAreaComparision(areaIds: string[]) {
    ...
    array.forEach(element => {
      ...
      switch (element.area.areaLevel.level) {
      ...
      if (recSource.slugidsource == element.src.slugidsource) {
        ...
        let areaData = {
          ...
          indicator: element.indicator.name,
