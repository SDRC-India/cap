...
try{
  data=((await this.http.get('assets/data/'+areaCode+'.json').toPromise()) as IDBData[])
      ...
      if(type==SnpashotProfile.RMNCHA)
        ...
        {
          // needs to be changed
        array = data.filter(d=>d.dTHEMATICRSrs==true && d.indicator.recSector.id==sector.id);
        }
        ...
        array.forEach(element => {
          ...
          else
          {
            let snapshotIndicatorData: ISnapshotIndicator = {
              ...
              source: element.src,
              trend: element.trend,
              theme: element.indicator.recSector.name,
    ...
    array.forEach(data => {
      ...
      if (count < 6) {
        let lineChart: LineChart = {
          ...
          value: data.value,
          zaxis: area.areaname,
          class: data.indicator.recSector.name
        }
  ...
  private orderByTheme(snapshotIndicatorList: ISnapshotIndicator[]): ISnapshotIndicator[] {
    ...
    let maternal: ISnapshotIndicator[] = snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().themeName_maternal).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
    ...
    let reproductive: ISnapshotIndicator[] = snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().themeName_reproductive).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
      if (a.indicator.name > b.indicator.name) {
        return 1
      } else if (a.indicator.name < b.indicator.name) {
        return -1
    ...
    let childcare: ISnapshotIndicator[] = snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().themeName_childcare).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
      if (a.indicator.name > b.indicator.name) {
        return 1
      } else if (a.indicator.name < b.indicator.name) {
        return -1
    ...
    let neonatal: ISnapshotIndicator[] = snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().themeName_neonatal).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
      if (a.indicator.name > b.indicator.name) {
        return 1
      } else if (a.indicator.name < b.indicator.name) {
        return -1
    ...
    let infra: ISnapshotIndicator[] =snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().health_infrastructure).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
      if (a.indicator.name > b.indicator.name) {
        return 1
      } else if (a.indicator.name < b.indicator.name) {
        return -1
    ...
    let hr: ISnapshotIndicator[] = snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().human_resource).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
      if (a.indicator.name > b.indicator.name) {
        return 1
      } else if (a.indicator.name < b.indicator.name) {
        return -1
    ...
    let adlosecent: ISnapshotIndicator[] = snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().adolescent).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
      if (a.indicator.name > b.indicator.name) {
        return 1
      } else if (a.indicator.name < b.indicator.name) {
        return -1
    ...
    let demography: ISnapshotIndicator[] = snapshotIndicatorList.filter(d => d.indicator.recSector.name === this.constantService.getConstantObject().demography).sort((a: ISnapshotIndicator, b: ISnapshotIndicator) => {
      if (a.indicator.name > b.indicator.name) {
        return 1
      } else if (a.indicator.name < b.indicator.name) {
        return -1
  ...
  getAreaText() {
    ...
    switch (selectedArea.areaLevel.level) {
