...
export class MyApp {
  ...
  constructor(
    ...
    this.pages = [
      ...
      { title: "Themes", component: "ThematicProfileViewPage" },
      { title: "Department", component: "DataRepositoryPage" },
      // { title: "Factsheet ", component: "FactsheetPage" },
      // { title: "Feedback ", component: "FeedbackPage" }
    ];

    this.syncService.logout.subscribe(flag => {
      if(flag){
  ...
  initializeApp() {
    this.platform.ready().then(() => {
      ...
      this.headerColor.tint("#d85c54");
      this.statusBar.overlaysWebView(false);

      // set status bar to white
      this.statusBar.backgroundColorByHexString("#d85c54");
...
console.log(dbSetUpJob);
    ...
    if (dbSetUpJob) {
      ...
      await this.pouchdbService.loadIndicator();
      await this.pouchdbService.loadArea();
      await this.pouchdbService.loadSource();
      await this.pouchdbService.loadSubGroup();
      // this.utilService.stopLoader()
      // await this.pouchdbService.writeCompressedDataToJsonFile();
    } else {
      this.utilService.stopLoader();
  ...
  openFile() {
    if (this.platform.is("cordova")) {
      ...
      window.open('assets/'+this.constantService.getConstantObject().userGuidePath);
    }
  }
}
